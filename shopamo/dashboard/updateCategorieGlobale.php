...
if(isset($_POST['Modifier'])){
    ...
    $IdCategorieGlob=$_POST['idCategorieGlob'];
    if(!empty($NameCategGlob) && !empty($IdCategorieGlob)){
        $con=Database::connect();
        $stmt = $con->prepare("UPDATE  categorieGlobale SET categorieGlobaleName=? WHERE categorieGlobaleId=?");
        $stmt->bindParam(1,$NameCategGlob);
        $stmt->bindParam(2,$IdCategorieGlob);
        $stmt->execute(); 
...
if(!empty($_GET["idCategorieGlobale"])){
    $idCategorieGlobale=$_REQUEST["idCategorieGlobale"];
    $con=Database::connect();
    $stmt=$con->query("SELECT * FROM categorieGlobale WHERE categorieGlobaleId=$idCategorieGlobale");
    $data=$stmt->fetch();
}
?>
<!DOCTYPE html>
...
<body style="margin: 0;">
    ...
    echo '<form method="post" enctype="multipart/form-data" action="" style="text-align:center;">';
        echo '<label>Nom de categorie globale: <input type="text" name="categorieGlobale" id="categorieGlobale" placeholder="Nom de categorie globale" style="width:27%; padding-top:6px; padding-bottom:6px;" value="'.$data["categorieGlobaleName"].'" required></label> '; 
        echo '<input type="hidden" value="'.$data["categorieGlobaleId"].'" name="idCategorieGlob">';
        echo '<button name="Modifier" class="btn" type="submit" style="height:30px: width: 6px; width:10%;"> Modifier</button>';
    echo '</form>';
            ...
            echo "</tr>";
                while($line=$sql->fetch()){
                    ...
                    echo '<td >';
                            ...
                            echo '<a href="deleteCategorieGlobale.php?idCategorieGlobale='.$line['categorieGlobaleId'].'"><img src="dashboard icons/delete.svg" alt="Supprimer"></a>';  
                    echo '</td>'; 
                echo "</tr>"; 
                }
                $con=Database::disconnect();
